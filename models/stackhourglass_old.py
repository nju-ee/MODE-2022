    ...
    def forward(self, left, right):
        ...
        if self.training:
            ...
            if self.out_conf:
                ...
                grid_d = torch.round(pred3).permute([0, 2, 3, 1]).unsqueeze(1) / (self.maxdisp-1) * 2 - 1
                ...
                grid_h = (grid_h / (grid_d.shape[2]-1)*2-1).unsqueeze(0).unsqueeze(-1).unsqueeze(0).repeat_interleave(grid_d.shape[0], dim=0).cuda()
                grid_w = (grid_w / (grid_d.shape[3]-1)*2-1).unsqueeze(0).unsqueeze(-1).unsqueeze(0).repeat_interleave(grid_d.shape[0], dim=0).cuda()

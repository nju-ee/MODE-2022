...
class PSMNet(nn.Module):
    def __init__(self, maxdisp, out_conf=True):
    ...
    def forward(self, left, right):
        ...
        if self.training:
            ...
            if self.out_conf:
                ...
                grid_d_floor = (torch.round(pred3)-1).permute([0, 2, 3, 1]).unsqueeze(1) / (self.maxdisp-1.0) * 2 - 1
                grid_d_ceil = (torch.round(pred3)+1).permute([0, 2, 3, 1]).unsqueeze(1) / (self.maxdisp-1.0) * 2 - 1
                ...
                prob_map = F.grid_sample(prob_volume.unsqueeze(1), grid,align_corners=True, padding_mode='border', mode='nearest') + F.grid_sample(prob_volume.unsqueeze(1), grid_floor,align_corners=True, padding_mode='border', mode='nearest') + F.grid_sample(prob_volume.unsqueeze(1), grid_ceil,align_corners=True, padding_mode='border', mode='nearest')
...
class ModesDisparity(nn.Module):
    def __init__(self, maxdisp, conv='Sphere', in_height=1024, in_width=512, sphereType='Cassini', out_conf=True):
        super(ModesDisparity, self).__init__()
        self.maxdisp = maxdisp
        self.out_conf = out_conf
        if conv == 'Regular':
            self.feature_extraction = feature_extraction()
